  ...
  public CommandReply execute(ReDuber db, ConnectedUser user)
    ...
    try {
      ...
      if (status == ReDuber.Status.FALSE) {
        return CommandReply.noPermission("Chat does not exist or you are not a member of the chat");
        ...
        if (db.stringGet("chats."+this.chatId+".type").get().equals("DM")) {
          ...
          if (users.length != 2) {
            Log.error("Inconsistent state DM members length != 2", "MessageHandler", this);
          ...
          if (
            ...
            return CommandReply.noPermission("You are no longer friends with the other DM user");
          }
        }
